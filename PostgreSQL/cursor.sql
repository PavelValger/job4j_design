...
BEGIN;
DECLARE
    cursor_reverse SCROLL cursor for
                        select * from products;

FETCH LAST FROM cursor_reverse;

FETCH BACKWARD FROM cursor_reverse;

FETCH PRIOR FROM cursor_reverse;

MOVE BACKWARD 8 FROM cursor_reverse;

FETCH PRIOR FROM cursor_reverse;

MOVE BACKWARD 7 FROM cursor_reverse;

FETCH PRIOR FROM cursor_reverse;

CLOSE cursor_reverse;
commit;