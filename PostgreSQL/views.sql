...
insert into students (name) values ('Алексей Алексеев');
...
insert into books (name) values ('Трансерфинг реальности');
...
create view show_students_students_without_book_and_books_without_author as
select s.name as "Студент без книги", count(s.name) "Количество",
b.NAME as "Книги без автора"
from students s
    left join orders o on s.id = o.student_id
    full join books b on o.book_id = b.id
    left join authors a on b.author_id = a.id
    group by (s.name, b.id, b.NAME, a.id) having b.id is NULL OR a.id is null;

SELECT * FROM show_students_students_without_book_and_books_without_author;